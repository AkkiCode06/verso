...
struct NowPlayingView: View {
    ...
    private var desiredLayer: BackgroundLayer {
        if let motion = nowPlaying.motionArtwork, settings.motionArtworkEnabled,
           !settings.excludesMotion(artist: nowPlaying.current?.artist,
                                    album: nowPlaying.current?.album) {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            TimelineView(.animation) { timeline in
                ...
                // Kept steady: the shaders take this as `time`, where it drives
                // the slow breathing that must not lurch with the music.
                ...
                // A beat *turns* the flow rather than only hastening it.
                //
                // Pushing the rate up and only up reads as fast-forward: what
                // you notice is the effect skipping, not the music. Giving the
                // response a sign fixes that. `turn` drifts between -1 and 1
                // over a minute or two, so for one stretch the beats drive the
                // motion on and for the next they check it and ease it back --
                // the same energy, spent on direction instead of speed.
                //
                // Two incommensurable periods, so it never settles into an
                // obvious cycle.
                let turn = sin(now * 0.083) * 0.70 + sin(now * 0.041 + 1.9) * 0.30
                let veer = (0.55 * reactivity.pulse + 0.28 * reactivity.swell) * turn
                // Base drift is a little under 1 so the average pace is
                // unchanged; `VisualClock` still eases toward this over a
                // couple of seconds, so a reversal arrives as the motion
                // slowing, stopping and turning rather than snapping around.
                let flowPhase = VisualClock.flow.advance(
                    to: now,
                    rate: speed * (0.90 + veer * 1.5)
                )
                // The line patterns draw from the same clock as the shaders.
                // They used to run at a fixed rate and take no input from the
                // music at all -- eighteen styles that looked identical
                // whatever was playing. Halved, because a pen-and-ink drawing
                // reversing at full speed reads as a glitch where a fluid
                // reads as a current.
                let patternPhase = flowPhase * 0.5
    ...
    private func backgroundLayer(
        ...
        // The colour field drifted on a metronome and answered the music only
        // by leaning its blobs 5% wider -- effectively deaf, which made
        // "Colour only" the one mode that ignored what was playing. It now
        // orbits on the same clock as everything else, at a third the rate:
        // the blobs are large and slow, so they need far less of it than a
        // shader does to read as moving with the track.
        let colourFlow = flowPhase * 0.34
        switch layer {
        case .motion(let url):
            ZStack {
                ...
                MotionArtworkView(url: url, placement: settings.motionPlacement)
                    .ignoresSafeArea()
